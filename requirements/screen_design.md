...
> **技術選定の補足**: Figma は Flowbite で設計したが、実装では **daisyUI** を採用した。理由＝動きを Hotwire で実装するため Flowbite の JS が不要になり、daisyUI の方が ERB が短く読みやすいため。Figma はクラス一致ではなく **画面構造・要素の参照** として用いる。以降の各「使用コンポーネント」は daisyUI のクラス名で記載する。
...
CSS は daisyUI、動きは Hotwire（Turbo + Stimulus）で実装し、手書き JS は最小化する。

| 機能 | 実装方針 |
|---|---|
| 画面遷移・フォーム送信 | Turbo Drive（標準で有効） |
| 検索結果の部分更新 | Turbo Frames |
| 貸出 / 返却での状態バッジ更新 | Turbo Streams |
| 削除確認モーダル等の小さな開閉 | daisyUI の `modal`（純CSS）または Stimulus |
...
`navbar` / `input`＋`btn`（検索）/ `table` / `badge badge-success`・`badge badge-error`（状態）/ `btn`（操作）/ `join`＋`btn`（ページネーション）
...
`breadcrumbs` / `badge` / `btn`（`btn-primary`借りる・`btn-warning`返却・`btn-error`削除・`btn-outline`編集）/ 詳細情報は `table` または `dl`
...
`input` × 2 / `btn btn-primary` ／ `card` でフォームを囲む
...
`input` 各種 / `select`・`checkbox`（著者/タグ選択）/ `btn`
...
`input` / `btn btn-error`（削除）/ `modal`（`<dialog>` ベース・削除確認）
...
`table` / `badge`（`badge-warning`貸出中・`badge-success`返却済）/ `join`＋`btn`（ページネーション）
