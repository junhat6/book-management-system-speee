...
class RentalTest < ActiveSupport::TestCase
  test "空いている現物への貸出は有効" do
    rental = Rental.new(user: users(:one), book_item: book_items(:one_item_a))

    assert rental.valid?
  end

  test "貸出中の現物への貸出は無効" do
    rental = Rental.new(user: users(:one), book_item: book_items(:two_item_a))

    assert_not rental.valid?
    assert_includes rental.errors[:book_item], "は貸出中のため借りられません"
  end

  test "返却済みになった現物は再度借りられる" do
  ...
  test "同じ本を借りている間は別の現物も借りられない" do
    # users(:two) は books(:two) の現物Aを貸出中
    rental = Rental.new(user: users(:two), book_item: book_items(:two_item_b))

    assert_not rental.valid?
    assert_includes rental.errors[:base], "すでにこの本を借りています"
  end

  test "別のユーザーなら同じ本の別の現物を同時に借りられる" do
    rental = Rental.new(user: users(:one), book_item: book_items(:two_item_b))

    assert rental.valid?
  end

  test "モデルの検証を迂回してもDBの一意制約が現物の二重貸出を防ぐ" do
    bypass = Rental.new(user: users(:one), book_item: book_items(:two_item_a))
  ...
  test "sorted(\"returned_at\", \"asc\") で返却日の古い順に並び替えられる" do
    older = Rental.create!(user: users(:one), book_item: book_items(:one_item_a), returned_at: 15.days.ago)
